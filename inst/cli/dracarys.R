...
p <- argparse::ArgumentParser(description = "DRAGEN Output Tidying", prog = prog_nm)
p$add_argument("-v", "--version", action = "version", version = glue::glue("{prog_nm} {version}"))
...
if (length(args$subparser_name) == 0) {
  ...
  stop("Need to specify 'tidy' in the cli...")
}
