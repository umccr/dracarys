...
format:
  html:
    ...
    grid:
      body-width: 2800px
params:
  tidy_data: "~/projects/dracarys/inst/reports/wgts-qc/nogit/tidy_data_rds/2025-08-20_1to716_wgts.rds"
---
...
{
  use("dplyr")
  use("dracarys", "session_info_kable")
  use("DT", "datatable")
  use("forcats", "fct_rev")
  use("glue", "glue")
  use("here", "here")
...
d0 <- tidy_data_path |>
  ...
  mutate(
    umccrId = glue(
      "{.data$internal_subject_id}_{.data$libraryId}_{.data$lane}"
    ),
    umccrId = factor(.data$umccrId),
    projectOwnerName = glue("{.data$owner_id}_{.data$project_id}")
  ) |>
  select(
    "umccrId",
    subjectId = "internal_subject_id",
    "libraryId",
    "lane",
    "projectOwnerName",
    "sampleType",
...
dr_unnest <- function(x1, ...) {
  d_name |>
    dplyr::mutate(tidy_name = sub("dragen_", "", .data$tidy_name)) |>
...
d_map |>
  ...
  DT::formatCurrency(
    columns = names(d_map)[num_pct_cols],
    currency = "",
    digits = 1
  ) |>
  DT::formatCurrency(
    columns = names(d_map)[no_numpct_cols],
    currency = "",
    digits = 0
  ) |>
  DT::formatStyle(
...
d_time_plot <- d_time |>
  select(umccrId, type, phenotype, total_runtime:dplyr::last_col()) |>
  tidyr::pivot_longer(
    total_runtime:dplyr::last_col(),
    names_to = "STEP",
    values_to = "RUNTIME_SEC"
  ) |>
...
f1_plot <- ggplot() +
  ...
  geom_line(
    data = f1,
    aes(x = q, y = prop, colour = umccrId, linetype = mate),
    linewidth = 1,
    show.legend = FALSE
  ) +
  ...
  labs(
    ...
    subtitle = glue(
      "Percentage of reads with average quality scores. Shows if\na subset of reads has poor quality."
    )
  )
...
gc_data_plot <- gc_data |>
  ...
  labs(
    ...
    subtitle = glue(
      "Total number of reads with each GC content\npercentile between 0% and 100%"
    )
  )
...
f1_plot <- ggplot() +
  ...
  geom_line(
    data = f1,
    aes(x = pct, y = value, colour = umccrId, linetype = mate),
    linewidth = 1
  ) +
  facet_wrap(~type, ncol = 1) +
  labs(
    ...
    subtitle = glue(
      "Average Phred-scale read mean quality for reads with\neach GC content percentile between 0% and 100%."
    )
  )
plotly::ggplotly(f1_plot)
# f1_plot
...
ggplot() +
  geom_rect(
    ...
    fill = rep(
      fqc_colours2$col,
      length(unique(f1$umccrId)) * length(unique(f1$mate))
    ),
    alpha = 0.7
  ) +
...
f1 |>
  ...
  labs(
    ...
    subtitle = glue(
      "Phred-scale quality value for bases at a given location and a\ngiven quantile of the distribution ({paste(quants, collapse = ', ')})"
    )
  )
```
