...
lims_rds <- here::here(glue("nogit/data_portal/lims/2023-06-11.rds"))
# lims_raw <- dracarys::glims_read()
# saveRDS(lims, file = lims_rds)
lims_raw <- readr::read_rds(lims_rds)
pmeta <- here::here("nogit/data_portal/workflows/2023-06-11_subset.csv") |>
  ...
  # get rid of a couple wf runs before bcl_convert
...
```{r vistime, fig.width=13, fig.height = 8}
p1 <- pmeta_sumy |>
...
pmeta |>
  ...
  dplyr::select(
    runfolder_name, SubjectID, LibraryID, SampleID, Phenotype, ExternalSubjectID,
...
pmeta |>
  ...
  dplyr::mutate(SubjectID = dplyr::if_else(is.na(.data$sbj_url), get_sbj_url(.data$SubjectID), .data$sbj_url)) |>
  dplyr::select(
    SubjectID, LibraryID, SampleID, Phenotype, ExternalSubjectID,
...
pmeta |>
  ...
  dplyr::mutate(SubjectID = dplyr::if_else(is.na(.data$sbj_url), get_sbj_url(.data$SubjectID), .data$sbj_url)) |>
  dplyr::select(
    SubjectID, LibraryID, SampleID, Phenotype, Lane, ExternalSubjectID,
    ProjectOwner, ProjectName, Type, Assay, Source, Quality, Workflow,
...
pmeta |>
  ...
  dplyr::mutate(SubjectID = dplyr::if_else(is.na(.data$sbj_url), get_sbj_url(.data$SubjectID), .data$sbj_url)) |>
  dplyr::select(
    SubjectID, LibraryID, SampleID, Phenotype, ExternalSubjectID, ProjectOwner, ProjectName,
...
pmeta |>
  ...
  dplyr::mutate(SubjectID = dplyr::if_else(is.na(.data$sbj_url), get_sbj_url(.data$SubjectID), .data$sbj_url)) |>
  dplyr::select(
    SubjectID, LibraryID, SampleID, Phenotype, rnasum_dataset, ExternalSubjectID,
    ProjectOwner, ProjectName, Type, Assay, Source, Quality, Workflow,
