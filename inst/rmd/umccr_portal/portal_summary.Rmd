...
```{r load_pkgs}
{
  ...
  require(aws.signature, include.only = "locate_credentials")
  require(dplyr)
  require(dracarys)
...
```{r funcs}
get_workflows <- function(rows = 100) {
  url1 <- utils::URLencode(glue("https://api.portal.prod.umccr.org/iam/workflows?rowsPerPage={rows}"))
...
lims_rds <- here::here(glue("nogit/data_portal/lims/{today}.rds"))
# lims_raw <- glims_read()
# saveRDS(lims_raw, file = lims_rds)
lims_raw <- readr::read_rds(lims_rds)
pmeta_rds <- here::here(glue("nogit/data_portal/workflows/{today}.rds"))
# pmeta_raw <- get_workflows(60)
# saveRDS(pmeta, file = pmeta_rds)
...
pmeta <- pmeta_raw |>
  ...
  dplyr::mutate(
    # portal has UTC timezone
    start = as.POSIXct(.data$start, format = date_fmt, tz = utc_tz),
    end = as.POSIXct(.data$end, format = date_fmt, tz = utc_tz),
...
```{r vistime, fig.width=15, fig.height = 12}
p1 <- pmeta_sumy |>
