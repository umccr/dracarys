...
# options(width = 150)
fmt1 <- "%Y-%m-%dT%H:%M:%S"
date_start <- as.POSIXct("2023-12-20T00:00:01", format = fmt1)
date_end <- as.POSIXct("2023-12-24T23:59:59", format = fmt1)
wf_order <- c(
  "bcl_convert",
...
wf_query <- 'WHERE "start" > date(\'2023-12-19\') AND "start" < date(\'2023-12-25\') ORDER BY "start" DESC;'
...
```{r vistime, fig.width=15, fig.height = 15}
p1 <- pmeta_sumy |>
...
```{r eval=chunks1[[wf]]}
pmeta |>
  dracarys::meta_bcl_convert(status = c("Succeeded", "Running")) |>
  dplyr::select(
  ...
  dplyr::select(
    end_status, durationMin, runfolder_name, SubjectID, LibraryID, SampleID, Phenotype, ExternalSubjectID,
