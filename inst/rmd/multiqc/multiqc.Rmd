...
sel_cols <- c(
  ...
  "cov_auto_median_dragen", "cov_x_median_dragen", "cov_y_median_dragen",
  "contamination_conpair"
)
p <- d2 |>
  tidyr::pivot_longer(dplyr::all_of(sel_cols), names_to = "metric", values_to = "value")
p1 <- p |>
  dplyr::select(
    SubjectID, SampleID, umccr_workflow, date_portal, date_multiqc,
    ProjectOwner, ProjectName, Quality, Type, Phenotype,
    metric, value
  ) |>
  dplyr::mutate(
    latest_run = date_portal == "2023-04-23",
    sbj_samp = glue("{SubjectID}_{SampleID}_{Phenotype}")
  ) |>
  # filter(ProjectName %in% c("CIRCUIT", "Control", "PROSPECT")) |>
  filter(
    metric %in% c("contamination_conpair"),
    umccr_workflow == "dragen_umccrise",
    Phenotype %in% c("tumor", "normal")
  ) |>
  ggplot(aes(x = date_multiqc, y = value, text = sbj_samp)) +
  geom_point(aes(colour = latest_run)) +
  theme_bw() +
  ggtitle("Conpair Contamination")
# facet_wrap(workflow~Phenotype + metric, scales = "free")
plotly::ggplotly(p1, tooltip = c("x", "y", "text")) |>
  plotly::partial_bundle() |>
  htmlwidgets::saveWidget("~/tmp/conpair_contamination_2023-04-24.html")
```
