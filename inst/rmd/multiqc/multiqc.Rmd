...
p1 <- p |>
  ...
  dplyr::mutate(latest_run = date_portal == "2023-04-23", sbj_samp = glue("{SubjectID}_{SampleID}")) |>
  # filter(ProjectName %in% c("CIRCUIT", "Control", "PROSPECT")) |>
  filter(
    metric %in% c("reads_num_dupmarked_pct_dragen"),
    umccr_workflow == "dragen_alignment",
    Phenotype != "-"
  ) |>
  ggplot(aes(x = date_multiqc, y = value, text = sbj_samp)) +
  geom_point(aes(colour = latest_run)) +
  theme_bw() +
  facet_wrap(~Phenotype, scales = "fixed", ncol = 1)
