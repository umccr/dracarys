...
lims <- readr::read_tsv("~/Downloads/Google LIMS - Sheet1.tsv") |>
  dplyr::select(SubjectID, SampleID, LibraryID, Phenotype, ProjectOwner, ProjectName, Type, Workflow, Quality) |>
  dplyr::distinct()
dat <- dplyr::bind_rows(l1, .id = "sbj_date_hash") |>
  tidyr::separate(col = sbj_date_hash, sep = "_", into = c("umccr_sbj", "date1", "hash1")) |>
  dplyr::mutate(date = lubridate::ymd_hm(sub("_", " ", date))) |>
  dplyr::select(umccr_sbj, umccr_id, date, hash1, everything(), -date1) |>
  dplyr::left_join(lims |> select(SampleID, Phenotype) |> distinct(), by = c("umccr_id" = "SampleID"), multiple = "all") |>
  dplyr::arrange(umccr_sbj, date)
# dat |>
