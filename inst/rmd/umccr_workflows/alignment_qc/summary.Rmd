...
d_cvg <- dr_unnest("WgsCoverageMetricsFile") |>
  arrange(desc(umccrid)) |>
  left_join(d_pl_metrics, by = c("umccrid", "phenotype")) |>
  select(
    umccrid, phenotype,
    ploidy,
...
- Plot names used in MultiQC report are shown in ('parentheses').
...
f1_plot <- ggplot() +
  geom_rect(
    mapping = aes(xmin = fqc_colours1$start, xmax = fqc_colours1$end, ymin = 0, ymax = max(f1$prop) + 2),
    fill = fqc_colours1$col,
    alpha = 0.4
  ) +
...
ggplot() +
  geom_rect(
    ...
    fill = rep(fqc_colours2$col, length(unique(f1$umccrid))),
    alpha = 0.4
  ) +
...
d1 <- dr_unnest("WgsContigMeanCovFile") |>
  arrange(desc("umccrid"))
for (s in sort(unique(d1$umccrid), decreasing = TRUE)) {
  p1 <- d1 |>
    filter(umccrid == s) |>
...
plotly::subplot(d_hist1, d_hist2, shareY = TRUE, titleY = TRUE, titleX = TRUE, nrows = 1)
```
