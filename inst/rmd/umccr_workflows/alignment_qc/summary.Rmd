...
params:
  ...
  meta: !r here::here("inst/rmd/umccr_workflows/alignment_qc/nogit/meta/2023-12-09.rds")
...
stopifnot(all(dir.exists(meta$indir)))
...
dat <- meta |>
  rowwise() |>
  mutate(
    fpaths = list(filepaths(indir = .data$outdir, sampleid = .data$SampleID, lane = .data$Lane)),
    umccrid = glue("{.data$SubjectID}_{.data$LibraryID}_LN{.data$Lane}")
  ) |>
  select("umccrid", "phenotype", "fpaths") |>
  tidyr::unnest(fpaths) |>
  rowwise() |>
  mutate(
...
d <- dat |>
  select("umccrid", "phenotype", "ftype", "dat") |>
  tidyr::nest(data = c("umccrid", "phenotype", "dat"))
# sample-specific access
d_samp <- dat |>
  select("umccrid", "phenotype", "ftype", "dat") |>
  tidyr::nest(data = c("phenotype", "ftype", "dat")) |>
  arrange("umccrid")
```
...
dr_unnest <- function(x1) {
  d |>
    ...
    filter(nrows > 0) |>
    tidyr::unnest(dat) |>
    select("umccrid", "phenotype", everything(), -c("ftype", "nrows"))
}
...
d1 <- dr_unnest("MappingMetricsFile") |>
  ...
  select(
    ...
    insert_len_mean = insert_len_mean_dragen, everything()
  )
num_cols <- purrr::map_lgl(d1, is.numeric)
num_pct_cols <- grepl("%", names(d1)) & num_cols
no_numpct_cols <- !grepl("%", names(d1)) & num_cols
conf <- list(
  bg_size = "95% 90%",
  bg_repeat = "no-repeat",
  bg_pos = "center",
...
d1 |>
  dt_view() |>
  DT::formatCurrency(columns = names(d1)[num_pct_cols], currency = "", digits = 1) |>
  DT::formatCurrency(columns = names(d1)[no_numpct_cols], currency = "", digits = 0) |>
  DT::formatStyle(
...
num_cols <- names(d1)[purrr::map_lgl(d1, is.numeric)]
d1 |>
  dt_view() |>
...
d1_metrics <- d1 |>
  select(
    ...
    cvg_auto_med = cov_auto_median_dragen,
    cvg_x_med = cov_x_median_dragen,
    cov_y_med = cov_y_median_dragen
  )
d1_metrics |>
  dt_view()
...
d1_plot <- d1 |>
  ...
  tidyr::pivot_longer(-c("umccrid", "phenotype")) |>
  ...
  select(umccrid, phenotype, chrom, value)

d1_plot |>
  ggplot(aes(x = chrom, y = value, colour = umccrid, group = umccrid)) +
