...
pmeta <- here("nogit/data_portal/workflows_2023-06-10.csv") |>
...
rdf <- dat1 |>
  dplyr::filter(type == "multiqc") |>
  dplyr::select(
    wfr_id, portal_run_id,
    SubjectID, LibraryID,
    objp
    # start, end,
    # type, bname, size,
...
mqc1 <- dplyr::copy_to(sc, df = rdf, name = "multiqc1", overwrite = T)
mqc2 <- sparklyr::spark_write_table(mqc1, path = "multiqc_table2", name = "multiqc2")
