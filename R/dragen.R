#' Read DRAGEN `roh.bed` File
#'
#' Reads roh bed file.
#'
#' @param x Path to file.
#'
#' @return Tibble with roh regions, score, and number of homs/hets.
#'
#' @export
dragen_rohbed_read <- function(x) {
  d <- readr::read_tsv(
    x,
    col_types = "cddddd",
    col_names = c("chrom", "start", "end", "score", "hom", "het")
  )
  d[]
}

#' Read DRAGEN `roh_metrics.csv` File
#'
#' Reads roh metrics file.
#'
#' @param x Path to file.
#'
#' @return Tibble with pct_snvs, n_regions.
#'
#' @export
dragen_rohmetrics_read <- function(x) {
  d <- readr::read_lines(x)
  stopifnot("First line not starting with VARIANT CALLER" = grepl("VARIANT CALLER", d[1]))
  abbrev_nm <- c(
    "Percent SNVs in large ROH ( >= 3000000)" = "pct_snvs",
    "Number of large ROH ( >= 3000000)" = "n_regions"
  )
  res <- d |>
    tibble::as_tibble_col(column_name = "value") |>
    tidyr::separate_wider_delim(
      "value",
      names = c("varcall", "sample", "var", "value"),
      delim = ","
    ) |>
    dplyr::mutate(
...
dragen_covreportbed_read <- function(x) {
  d <- readr::read_tsv(x, col_types = "cddccddd") |>
...
#' Read DRAGEN Heterozygous/Homozygous Ratio File
#'
#' Reads the `vc_hethom_ratio_metrics.csv` file output from DRAGEN.
#'
#' @param x Path to file.
#'
#' @returns Tibble with metrics.
#'
#' @export
dragen_hethomratio_read <- function(x) {
  d <- readr::read_lines(x)
  stopifnot("First line not starting with VARIANT CALLER" = grepl("VARIANT CALLER", d[1]))
  abbrev_nm <- c(
    "Heterozygous" = "het",
  ...
  res <- d |>
    ...
    dplyr::mutate(
      value = as.numeric(.data$value),
      var = dplyr::recode(.data$var1, !!!abbrev_nm),
      varcall = sub("VARIANT CALLER ", "", .data$varcall)
    ) |>
...
#' Read DRAGEN Ploidy VCF
#'
#' Reads the `ploidy.vcf.gz` file generated by DRAGEN.
#' @param x Path to file.
#'
#' @return Tibble with depth of coverage (dc) and normalised depth of coverage (ndc)
#' for each chromosome.
#' @export
dragen_ploidy_vcf_read <- function(x) {
  dat <- bcftools_parse_vcf(x, only_pass = FALSE, alias = TRUE)
  colnames(dat) <- tolower(colnames(dat))
  d <- dat |>
    dplyr::select("chrom", "alt", "qual", "filter", "info_svtype", dc = "s1_dc", ndc = "s1_ndc")
  # ensure proper classes of columns in case of all NAs
  d |>
    dplyr::mutate(
      alt = as.character(.data$alt),
      info_svtype = as.character(.data$info_svtype)
    )
}
...
#' Read DRAGEN Targeted VCF
#'
#' Reads the `targeted.vcf.gz` file generated by DRAGEN.
#' @param x Path to file.
#'
#' @return Tibble with VCF fields.
#' @export
dragen_targeted_vcf_read <- function(x) {
  dat <- bcftools_parse_vcf(x, only_pass = TRUE, alias = TRUE)
  colnames(dat) <- tolower(colnames(dat))
  ...
  public = list(
    ...
    initialize = function(path = NULL, prefix = NULL) {
      ...
      regexes <- tibble::tribble(
        ...
        glue("{pref}\\.vc_hethom_ratio_metrics\\.csv$")   , "read_hethomRatio"        ,
    ...
    #' @description Read `roh.bed` file.
    #' @param x Path to file.
    read_rohBed = function(x) {
      dat <- dragen_rohbed_read(x)
      tibble::tibble(name = "dragen_rohbed", data = list(dat[]))
    },
    #' @description Read `roh_metrics.csv` file.
    #' @param x Path to file.
    read_rohMetrics = function(x) {
      dat <- dragen_rohmetrics_read(x)
      tibble::tibble(name = "dragen_rohmetrics", data = list(dat[]))
    },
    ...
    #' @description Read `ploidy.vcf.gz` file.
    #' @param x Path to file.
    read_ploidyVcf = function(x) {
      dat <- dragen_ploidy_vcf_read(x)
      tibble::tibble(name = "dragen_ploidyvcf", data = list(dat[]))
    },
    ...
    #' @description Read `vc_hethom_ratio_metrics.csv` file.
    #' @param x Path to file.
    read_hethomRatio = function(x) {
      dat <- dragen_hethomratio_read(x)
      tibble::tibble(name = "dragen_hethomratio", data = list(dat[]))
    }
  ), # end public
  private = list(
